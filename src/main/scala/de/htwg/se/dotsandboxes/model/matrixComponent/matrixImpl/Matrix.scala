...
case class Matrix[T](vecStatus: Vector[Vector[Any]], vecRow: Vector[Vector[Any]], vecCol: Vector[Vector[Any]], list: Vector[Player], currentPlayer: Player) extends MatrixInterface[T]:
  def this(rowSize: Int, colSize: Int, status: Any, playerSize: Int = 2) =
    this(Vector.tabulate(colSize, rowSize) { (_, _) => status }, Vector.tabulate(colSize + 1, rowSize) { (_, _) => false }, Vector.tabulate(colSize, rowSize + 1) { (_, _) => false }, new PlayerList(playerSize).playerList, list.head)
  ...
  override def checkSquare(thisCase: String, x: Int, y: Int): Matrix[Any] = thisCase match
    case "downcase" =>
      if ((cell(1, x + 1, y), cell(2, x, y), cell(2, x, y + 1)).toList.forall(_ == true))
        replaceCell(0, x, y, currentPlayer.status)
      else copy()
    case "upcase" =>
      if ((cell(1, x - 1, y), cell(2, x - 1, y), cell(2, x - 1, y + 1)).toList.forall(_ == true))
        replaceCell(0, x - 1, y, currentPlayer.status)
      else copy()
    case "rightcase" =>
      if ((cell(2, x, y + 1), cell(1, x, y), cell(1, x + 1, y)).toList.forall(_ == true))
        replaceCell(0, x, y, currentPlayer.status)
      else copy()
    case "leftcase" =>
      if ((cell(2, x, y - 1), cell(1, x, y - 1), cell(1, x + 1, y - 1)).toList.forall(_ == true))
        replaceCell(0, x, y - 1, currentPlayer.status)
      else copy()
