...
trait ComputerInterface:
  ...
  def isClosingMove(field: FieldInterface, vec: Int, x: Int, y: Int): Boolean =
    val squareCases: Vector[SquareState] = getSquareCases(vec, x, y, field)
  ...
  def getSquareCases(vec: Int, x: Int, y: Int, field: FieldInterface): Vector[SquareState] = vec match
    case 1 =>
      Vector(
        Option.when(x >= 0 && x < field.maxPosX)(SquareState.DownCase),
        Option.when(x > 0 && x <= field.maxPosX)(SquareState.UpCase)
      ).flatten
    case 2 =>
      Vector(
        Option.when(y >= 0 && y < field.maxPosY)(SquareState.RightCase),
        Option.when(y > 0 && y <= field.maxPosY)(SquareState.LeftCase)
      ).flatten
